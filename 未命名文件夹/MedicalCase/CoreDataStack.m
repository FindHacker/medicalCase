...
#pragma mask -  medical case create


#pragma mask - medical case management
///create docotor only for a doctor
-(void)createDoctorEntityWithDataDic:(NSDictionary*)dataDic
{
    NSEntityDescription *entityDesc = [NSEntityDescription entityForName: [Doctor entityName]inManagedObjectContext:self.managedObjectContext];
    Doctor *doctor = [[Doctor alloc] initWithEntity:entityDesc insertIntoManagedObjectContext:self.managedObjectContext];
...
///-(void)createPatientEntityWithDataDic:(NSDictionary*)dataDic doctorName:(NSString*)dName doctorID:(NSString*)dID doctorIsResident:(BOOL)isResident doctorIsAttendingPhysician:(BOOL) isAttendingPhysician doctorIsChiefPhysician:(BOOL)isChiefPhysician
-(void)createPatientEntityWithDataDic:(NSDictionary*)dataDic doctorInfo:(NSDictionary*)doctorDic
 //                          doctorName:(NSString*)dName doctorID:(NSString*)dID doctorIsResident:(BOOL)isResident doctorIsAttendingPhysician:(BOOL) isAttendingPhysician doctorIsChiefPhysician:(BOOL)isChiefPhysician
{
    ...
    if ([doctorDic.allKeys containsObject:@"isResident"]) {
        
    }
    if ([doctorDic.allKeys containsObject:@"isAttendingPhysician"]) {
        
    }
    if ([doctorDic.allKeys containsObject:@"isChiefPhysician"]) {
        
    }

    NSString *dName = [doctorDic objectForKey:@"dName"];
    NSString *dID = [doctorDic objectForKey:@"dID"];
    BOOL isResident = (BOOL)[doctorDic objectForKey:@"isResident"];
    BOOL isAttendingPhysician =(BOOL) [doctorDic objectForKey:@"isAttendingPhysician"];
    BOOL isChiefPhysician =(BOOL) [doctorDic objectForKey:@"isChiefPhysician"];
    
    NSArray *tempArray = [self fetchDoctorEntityWithName:dName dID:dID];
    
    if (tempArray.count == 1) {
        
        Doctor *doctor = (Doctor*)[tempArray firstObject];
        
        doctor.isAttendingPhysican = [NSNumber numberWithBool:isResident];
        doctor.isResident = [NSNumber numberWithBool:isAttendingPhysician];
        doctor.isChiefPhysician = [NSNumber numberWithBool:isChiefPhysician];
...
-(NSArray*)fetchDoctorEntityWithName:(NSString*)entityName dID:(NSString*)dID
{
    ...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"dName = %@,dID = %@",entityName,dID];
